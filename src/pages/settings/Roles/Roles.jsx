...
const RolesPage = () => {
  ...
  const { data: tableData = [], loading, error } = useSelector((state) => state.role || {});
  ...
  const handleAdd = async (values, resetForm) => {
    setIsSaving(true);
    try {
      const res = await dispatch(addRole({ name: values.roleName, status: 1 }));
      setIsSaving(false);
      if (res.error) {
  ...
  const handleEditSubmit = async (values, resetForm) => {
    ...
    try {
      const res = await dispatch(updateRole({ id: editData.id, name: values.roleName }));
  ...
  const downloadCSV = () => {
    const headers = ["Role Name", "Status"];
    const rows = (tableData || []).map((r) => "${r.name}");
    const csvContent = [headers.join(","), ...rows].join("\n");
    const blob = new Blob([csvContent], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", "roles_data.csv");
  ...
  return (
    <>
          ...
          >
            <MaterialReactTable
              ...
              muiTableBodyRowProps={({ row }) => ({
                hover: false,
                sx: row.original.status === "inactive" ? { "&:hover": { backgroundColor: "transparent" } } : {},
              })}
      ...
      <BootstrapDialog onClose={handleClose} open={open} fullWidth maxWidth="xs">
        <DialogTitle sx={{ m: 0, p: 1.5 }}>Add Role</DialogTitle>
        <IconButton
          aria-label="close"
          onClick={handleClose}
      ...
      <BootstrapDialog onClose={handleEditClose} open={editOpen} fullWidth maxWidth="xs">
        <DialogTitle sx={{ m: 0, p: 1.5 }}>Edit Role</DialogTitle>
        <IconButton
          aria-label="close"
        ...
        <Formik
          enableReinitialize
          initialValues={{ roleName: editData?.name || "" }}
          validationSchema={validationSchema}
