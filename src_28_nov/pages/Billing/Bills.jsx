...
const Bills = () => {
    ...
    const { data: bills = [], loading } = useSelector((state) => state.bill);
    console.log(bills);
    ...
    const columns = useMemo(
        () => [
            ...
            {
                accessorKey: "customer_mobile", header: "Mobile", size: 120,
                ...
                Cell: ({ row }) => (
                    <Box sx={{ display: "flex", gap: 1, justifyContent: "flex-end" }}>
                        ...
                        <Tooltip title="Delete">
                            <IconButton
                                color="error"
                                onClick={() => { setOpenDelete(true), setDeleteRow(row.original.id) }}
                            >
    ...
    return (
        <>
            <Grid container spacing={1}>
                <Grid size={12}>
                    <Paper elevation={0} sx={{ width: "100%", backgroundColor: "#fff", px: 2 }} ref={tableContainerRef}>
                        <MaterialReactTable
                            columns={columns}
                            data={bills}
                            enableTopToolbar
